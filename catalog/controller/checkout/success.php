...
class Success extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->session->data['order_id'])) {
			...
			// Push purchase events to AI right after a successful checkout.
			if ($order_id > 0) {
				...
				foreach ($order_products as $order_product) {
					...
					$quantity = max(1, (int)($order_product['quantity'] ?? 1));
					...
					for ($i = 0; $i < $quantity; $i++) {
						$this->model_tool_ai_event->logEvent($product_id, 'purchase');
					}
				}
			}
